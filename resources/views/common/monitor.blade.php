...
@section('content')
    ...
    {{-- <div class="mt-6 w-[1200px] rounded-xl bg-gray-600 p-6 text-white shadow-md">
...
{{-- 
    <div class="mt-6 w-full">
        <div class="w-[1200px] rounded-xl bg-gray-600 p-6 text-white shadow-md">
            <p class="text-lg font-semibold">Downtime Logs</p>

            @if (App\Models\WebsiteLog::all()->isEmpty())
                <p class="text-lg font-bold">No Downtime Logs Yet</p>
            @else
                <ul>
                    @foreach (App\Models\WebsiteLog::latest()->take(10)->get() as $log)
                        <li class="mt-2 flex justify-between border-b pb-2"> --}}
                            {{-- <span>{{ $log->website->name }} ({{ $log->website->url }})</span> --}}
                            {{-- <span class="font-bold {{ $log->status === 'Down' ? 'text-red-400' : ' text-green-400' }}">
                                {{ $log->status ? $log->status : 'No Status Available' }}
                            </span>
                            <span class="text-gray-300">{{ $log->logged_at }}</span>
                        </li>
                    @endforeach
                </ul>
            @endif
    ...
    {{-- <div class="mt-6 w-full">
        <div class="w-[1200px] rounded-xl bg-gray-600 p-6 text-white shadow-md">
            ...
            @else
                <ul> --}}
                    {{-- @foreach (App\Models\WebsiteLog::latest()->take(10)->get() as $log) --}}
                    {{-- @foreach (App\Models\WebsiteLog::where('status', 'Down')->get() as $log)
                        <li class="mt-2 flex justify-between border-b pb-2"> --}}
                            {{-- <span>{{ $log->website->name }} ({{ $log->website->url }})</span> --}}
                            {{-- <span class="font-bold {{ $log->status=== 'Down'? 'text-red-400' :' text-green-400'}}">
                                {{ $log->status ? $log->status : 'No Status Available' }}
