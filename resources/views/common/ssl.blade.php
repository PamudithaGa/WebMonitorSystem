...
@section('content')
    <div class="container mx-auto mt-[60px] p-6">
        ...
        <div class="overflow-x-auto p-4">
            <input id="search-input" type="search" placeholder="🔍 Search Website or Expiry Date"
                    ...
                    @foreach (App\Models\SslCertificate::all() as $ssl)
                        ...
                        @endauth --}}
                            @auth
                                @if (Auth::user()->email === 'pamudithagangana45@gmail.com' && $ssl->alert_sent >= 3)
                                    <td class="px-6 py-4">
                                        <form action="{{ route('website.ssl.activate', $ssl->website_id) }}" method="POST">
                                            @csrf
                                            <button type="submit"
                                                class="rounded-md bg-red-600 px-4 py-2 text-white transition hover:bg-red-800">
                                                Active SSL
                                            </button>
                                        </form>
                                    </td>
